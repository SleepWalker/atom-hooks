...
describe('#getVars', () => {
  it('should extract vars from file path', () => {
    ...
    atom.project.relativizePath.mockReturnValue(['/path/to/the/project', 'src/file.js']);
...
describe('#runCommand()', () => {
  it('runs command and interpolates vars', () => {
    ...
    return runner.runCommand('bar/baz', 'foo ${path}').then(() => {
      expect(runner.execute.mock.calls[0]).toEqual([expectedCommand, expectedOptions]);
    });
  });
});
...
describe('#run()', () => {
  it('runs all commands by path and hook', () => {
    ...
    commands.forEach(curCmd => exec.mockImplementationOnce((command, options, callback) => {
  ...
  it('rejects if one of commands have failed', () => {
    ...
    return runner.run(expectedFilePath, 'onSave').catch(() => expect(exec.mock.calls.length).toBe(1));
  });
});