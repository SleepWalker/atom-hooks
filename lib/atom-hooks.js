...
export default {
  ...
  activate() {
    ...
    this.subscriptions.add(
      atom.commands.add('atom-text-editor', 'atom-hooks:show', () =>
        this.onToggleHooksList()
      )
    );
    ...
    this.subscriptions.add(
      atom.workspace
        .getCenter()
        .observeActivePaneItem(item => this.onChangeActivePane(item))
    );
  ...
  onChangeActivePane(item: any) {
    if (
      !atom.workspace.isTextEditor(item) ||
      this.config.listHooks(this.getCurrentFile()).length === 0
    ) {
    ...
    if (!this.hooksListView) {
      this.hooksListView = new HooksListView({
        ...
        runCommand: (command: string) =>
          this.processExecutionResult(
            this.runner.runCommand(this.getCurrentFile(), command)
          )
      });
    }
  ...
  processExecutionResult(result: Promise<void>) {
    return result
