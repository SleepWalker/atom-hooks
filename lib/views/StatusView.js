...
type State = 'default' | 'loading' | 'success' | 'fail';
...
export default class StatusView {
  ...
  constructor() {
    ...
    link.addEventListener('click', (event: MouseEvent) =>
      this.onListHooks(event)
    );
    this.subscriptions.add(
      atom.tooltips.add(link, {
  ...
  updateState(state?: State = 'default') {
    this.state = state;
    this.icon.className = '';
  ...
  onListHooks(event: MouseEvent) {
    event.preventDefault();

    atom.commands.dispatch(
      atom.workspace.getCenter().getActivePaneItem().element,
      'atom-hooks:show'
    );
  }
